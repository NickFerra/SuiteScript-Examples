 ...
 *
 * Name:			{NPS Send Email} (npsSendEmail.js)
 *
 ...
 * Author:			RSM
 *
 * Script:			customscript_nps_send_email
 * Deploy:			customdeploy_nps_send_email
 *
 * Purpose:         Once a NPS Survey record is created this will send an email to the contact on that record asking them to fill out a survey.
 *
 * Notes:
 *
    ...
    (Library, nRuntime, nUrl, nRecord, nRender, nEmail) =>
    {
        ...
        const onAction = (context) =>
        {
            ...
            try
            {
                ...
                onlineFormUrl = script.getParameter({name: 'custscript_nps_onlineform'});
                emailTemplateParam = script.getParameter({name: 'custscript_nps_emailtemplate'});
        ...
        const formatUrl = (record, onlineFormUrl) =>
        {
            ...
            try
            {
                ...
                project = record.getValue({fieldId: 'custrecord_nps_project'});
                contact = record.getValue({fieldId: 'custrecord_nps_contact'});
                customer = record.getValue({fieldId: 'custrecord_nps_customer'});

                returnUrl = nUrl.format({
                    domain: onlineFormUrl,
                    params: {
                        custrecord_nps_survey_request:id,
                        custrecord_nps_resp_project:project,
                        custrecord_nps_resp_contact:contact,
                        custrecord_nps_resp_customer:customer,
                    }
                });
            }
            catch (e)
        ...
        const formatEmail = (record, emailTemplateParam, url) =>
        {
            ...
            try
            {
                ...
                contactField = record.getText({fieldId: 'custrecord_nps_contact'});
                projectField = record.getText({fieldId: 'custrecord_nps_project'});
                customerField = record.getText({fieldId: 'custrecord_nps_customer'});
                contactValue = record.getValue({fieldId: 'custrecord_nps_contact'});
